...
class ArcballControl {
  ...
  private readonly IDENTITY_QUAT: quat = quat.create();
  ...
  update(deltaTime: number, targetFrameDuration = 16): void {
    ...
    const snapRotation: quat = quat.create();
    ...
    this._rotationVelocity +=
      (rv - this._rotationVelocity) * RV_INTENSITY;
...
class InfiniteGridMenu {
  ...
  private discBuffers!: GeometryData;
  ...
  private init(onInit: ((sketch: InfiniteGridMenu) => void) | null): void {
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.multiply(
        matrix,
        matrix,
        mat4.targetTo(mat4.create(), [0, 0, 0], p as unknown as number[], [0, 1, 0])
      );
      mat4.multiply(
        matrix,
        matrix,
  ...
  private updateCameraMatrix(): void {
    mat4.targetTo(
      this.camera.matrix,
      this.camera.position as unknown as number[],
      [0, 0, 0],
      this.camera.up as unknown as number[]
    );
...
const INFINITE_MENU_STYLES = `
  ...
  .infinite-menu-action-button-icon {
    user-select: none;
    position: relative;
    color: #fff;
    top: 2px;
  ...
  .infinite-menu-face-title.active {
    opacity: 1;
    transform: translate(20%, -50%);
    pointer-events: auto;
    transition: 0.5s ease;
  }
  .infinite-menu-face-title.inactive {
    pointer-events: none;
    opacity: 0;
    transition: 0.1s ease;
  }
  .infinite-menu-face-description {
    user-select: none;
    position: absolute;
    max-width: 10ch;
    top: 50%;
  ...
  .infinite-menu-face-description.active {
    opacity: 1;
    transform: translate(-90%, -50%);
    pointer-events: auto;
    transition: 0.5s ease;
  }
  .infinite-menu-face-description.inactive {
    pointer-events: none;
    transform: translate(-60%, -50%);
    opacity: 0;
    transition: 0.1s ease;
  }
  .infinite-menu-action-button.active {
    bottom: 3.8em;
  ...
  .infinite-menu-action-button.inactive {
    bottom: -80px;
...
export default function InfiniteMenu({
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      <style>{INFINITE_MENU_STYLES}</style>

      <canvas className="infinite-menu-canvas" ref={canvasRef} />

      {activeItem && (
        <>
          <h2 className={`infinite-menu-face-title ${stateClass}`}>
            {activeItem.title}
          </h2>

          <p className={`infinite-menu-face-description ${stateClass}`}>
            {activeItem.description}
          </p>

          <div
            onClick={handleButtonClick}
            className={`infinite-menu-action-button ${stateClass}`}
          >
            <p className="infinite-menu-action-button-icon">&#x2197;</p>
          </div>
        </>
      )}
    </div>
