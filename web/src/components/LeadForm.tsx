    ...
    return (
      <SectionWrapper id="orcamento" className="section-warm py-14 sm:py-28">
        <div className="mx-auto max-w-2xl px-4 text-center sm:px-6">
          <div className="light-card rounded-2xl p-8 sm:p-12">
            <div
              className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full sm:mb-6 sm:h-16 sm:w-16"
            ...
            <h3 className="font-serif text-xl sm:text-2xl md:text-3xl" style={{ color: "#1a130a" }}>
              Pedido Recebido!
            </h3>
            <p className="mt-4 text-sm" style={{ color: "rgba(26,19,10,0.60)" }}>
              Vamos analisar o seu pedido e entrar em contacto nas próximas 24 horas.
            </p>
          </div>
        </div>
      </SectionWrapper>
  ...
  return (
    <SectionWrapper id="orcamento" className="section-warm py-14 sm:py-28">
      <div className="relative z-10 mx-auto max-w-2xl px-4 sm:px-6">
        ...
        <div className="mt-8 light-card rounded-2xl p-5 sm:mt-12 sm:p-8">
          {/* Progress bar */}
          <div className="mb-8">
            <div className="mb-2 flex justify-between text-xs" style={{ color: "rgba(26,19,10,0.50)" }}>
              <span>Passo {step} de 4</span>
              <span>{Math.round((step / 4) * 100)}%</span>
            </div>
            <div className="h-1 overflow-hidden rounded-full" style={{ background: "rgba(26,19,10,0.10)" }}>
              <motion.div
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="s1" variants={STEP_VARIANTS} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                <h3 className="mb-6 text-lg font-semibold" style={{ color: "#1a130a" }}>
                    ...
                    <button
                      ...
                      style={
                        formData.workType === wt.value
                          ? { borderColor: "#E67E22", background: "rgba(230,126,34,0.08)", color: "#E67E22" }
                          : { borderColor: "rgba(26,19,10,0.15)", color: "rgba(26,19,10,0.65)" }
                      }
                    >
                    ...
                    <button
                      ...
                      style={
                        formData.urgency === u.value
                          ? { borderColor: "#E67E22", background: "rgba(230,126,34,0.08)" }
                          : { borderColor: "rgba(26,19,10,0.15)" }
                      }
                    >
                      <p
                        className="font-semibold"
                        style={{ color: formData.urgency === u.value ? "#E67E22" : "#1a130a" }}
                      >
                        {u.label}
                      </p>
                      <p className="text-sm" style={{ color: "rgba(26,19,10,0.55)" }}>{u.sub}</p>
                    </button>
                  ))}
                </div>
              </motion.div>
                ...
                <div className="space-y-6">
                  <div>
                    <input
                      ...
                      className="w-full border-b bg-transparent px-0 py-3 text-sm placeholder:opacity-40 focus:outline-none"
                      style={{
                        borderColor: "rgba(26,19,10,0.20)",
                        color: "#1a130a",
                      }}
                    />
                    <p className="mt-2 text-xs" style={{ color: "rgba(26,19,10,0.45)" }}>
                      Pode indicar apenas a zona (ex: &ldquo;Benfica, Lisboa&rdquo;).
                    </p>
                  </div>
                  <div>
                    <label className="mb-1.5 block text-xs font-medium" style={{ color: "rgba(26,19,10,0.60)" }}>
                      Data preferencial para visita (opcional)
                    </label>
                    <input
                      type="date"
                      min={today}
                      ...
                      style={{
                        borderColor: "rgba(26,19,10,0.20)",
                        color: formData.preferredDate ? "#1a130a" : "rgba(26,19,10,0.40)",
              ...
              <motion.div key="s4" variants={STEP_VARIANTS} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}>
                ...
                <div className="space-y-5">
                  <input
                    ...
                    className="w-full border-b bg-transparent px-0 py-3 text-sm placeholder:opacity-40 focus:outline-none"
                    style={{ borderColor: "rgba(26,19,10,0.20)", color: "#1a130a" }}
                  />

                  {/* Contact method toggle */}
                  <div>
                    <p className="mb-2 text-xs font-medium" style={{ color: "rgba(26,19,10,0.60)" }}>
                      Prefere contacto via:
                    </p>
                        ...
                        <button
                          ...
                          style={
                            formData.contactMethod === method
                              ...
                              : { borderColor: "rgba(26,19,10,0.15)", color: "rgba(26,19,10,0.60)" }
                          }
                        >
                  ...
                  {formData.contactMethod === "whatsapp" ? (
                    <input
                      type="tel"
                      placeholder="Nº WhatsApp (ex: 912 345 678)"
                  ...
                  <div>
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed py-4 text-xs transition-all cursor-pointer hover:border-opacity-60"
                      style={{
                        borderColor: "rgba(26,19,10,0.20)",
                        color: "rgba(26,19,10,0.50)",
                      }}
                    >
          ...
          <div className="mt-8 flex items-center justify-between">
            {step > 1 ? (
              <button
                ...
                style={{ color: "rgba(26,19,10,0.50)" }}
              >
                &larr; Voltar
              </button>
            ) : (
            ...
            {step < 4 ? (
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="rounded-lg px-8 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer"
                style={{ background: "#E67E22", boxShadow: "0 4px 16px rgba(230,126,34,0.3)" }}
              >
                Continuar &rarr;
              </motion.button>
            ) : (
              <motion.button
                ...
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.97 }}
                className="w-full rounded-lg py-4 text-sm font-semibold tracking-[0.1em] uppercase text-white disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto sm:px-12 cursor-pointer"
                style={{ background: "#E67E22", boxShadow: "0 4px 16px rgba(230,126,34,0.3)" }}
              >
